...
function createMouseCloudProject() {
  const particles = [];
  const palette = [
    { rgb: hexToRgb("#1a98ff"), alpha: 0.75 },
  ...
  let mouseX = viewWidth * 0.5;
  let mouseY = viewHeight * 0.5;
  let mouseActive = false;
  let rotation = 0;
  let explodeEnergy = 0;
  ...
  function onKeyDown(event) {
    ...
    for (let i = 0; i < particles.length; i += 1) {
      ...
      const impulse = (260 + Math.random() * 520) / p.mass;
      const twist = (Math.random() - 0.5) * 140;

      p.vx += dx * impulse - dy * twist / p.mass;
      p.vy += dy * impulse + dx * twist / p.mass;
    }
  }
  ...
  function seedParticles() {
    ...
    const count = Math.round(clamp(viewWidth * viewHeight * 0.00065, 260, 900));
  ...
  return {
    ...
    render(timestamp, deltaSeconds) {
      ...
      for (let i = 0; i < particles.length; i += 1) {
        ...
        p.offsetAngle += p.offsetSpeed * deltaSeconds;
        const swirlAngle = p.offsetAngle + rotation * 0.7;
        ...
        const desiredX = mouseX + swirlX * p.offsetRadius;
        const desiredY = mouseY + swirlY * p.offsetRadius;

        const ax = (desiredX - p.x) * (p.spring / p.mass);
        const ay = (desiredY - p.y) * (p.spring / p.mass);

        p.vx += ax * deltaSeconds;
        p.vy += ay * deltaSeconds;

        const jitter = (0.08 + Math.random() * 0.08) * (1 + explodeEnergy * 0.8);
        ...
        if (p.x < -260) {
          p.x = viewWidth + 260;
        } else if (p.x > viewWidth + 260) {
          p.x = -260;
        }

        if (p.y < -260) {
          p.y = viewHeight + 260;
        } else if (p.y > viewHeight + 260) {
          p.y = -260;
        }
...
const projects = [createSinewaveProject(), createSpinningParticlesProject(), createMouseCloudProject()];
